...
Redmine::Plugin.register :redmine_wiki_html_util do
  ...
  Redmine::WikiFormatting::Macros.register do
    desc "Embed SVG file into the DOM\nUsage:\n<pre>{{svg(file.svg)}}</pre> searches svg file with specified name in `files` directory"
    macro :svg do |obj, args|
      raise 'No or bad arguments.' if args.size != 1
      _file_path = args.first
      _file = File.join(Rails.root, 'files', _file_path)
      if File.file?(_file)
        return File.read(_file).html_safe
      else
        return "<!-- no SVG file (#{_file_path}) found -->"
      end
    end
  end
